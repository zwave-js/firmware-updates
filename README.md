...
- Adds the **optional** `additionalFirmwareVersions` field per device. This is a record mapping firmware target indices (as strings) to version strings, used for multi-target firmware version gating in `$if` conditions (e.g. `firmwareVersion[1] >= 2.0`).
