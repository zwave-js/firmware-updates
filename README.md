...
Firmware upgrades are defined in the files in the `firmware` directory. This information is parsed by the web service and stored in a Cloudflare D1 database. The web service uses this database to respond to requests for firmware updates.
...
> **Note**: The service has been migrated from R2 file-based storage to D1 database storage for better performance and efficiency. The JSON files in the `firmware` directory are now uploaded and converted to database records.
